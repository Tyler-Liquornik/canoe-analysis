 ...
 */
@Traceable
public class BeamController implements Initializable, ModuleController {
    @FXML
    ...
    /**
     * Applies only the selected canoe's hull. GirRaft deliberately preserves
     * the original wrench behavior by scaling to the length already entered;
     * Raft Punk uses its fixed 5.90 m CAD-derived geometry.
     */
    private void loadCanoePreset(CanoePreset preset) {
        Hull presetHull = preset == CanoePreset.GIRRAFT_2025
                ? HullLibrary.generateGirRaftHullScaled(canoe.getHull().getLength())
                : preset.createFullSizeHull();
