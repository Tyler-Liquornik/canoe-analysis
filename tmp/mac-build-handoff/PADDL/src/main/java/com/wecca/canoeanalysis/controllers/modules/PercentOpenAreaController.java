...
public class PercentOpenAreaController implements Initializable, ModuleController {
    ...
    private void processImageFile(File file) {
        ...
        try {
            ...
            if (image != null) { // Checking if an image has been uploaded
                ...
                double reductionCoefficient = Math.min(ratioX, ratioY);
            ...
            mainController.showSnackbar("Error: File not found!");
        }
    }
    ...
    /** Loads GirRaft's recorded PADDL mesh result and competition threshold. */
    private void loadGirRaftPreset() {
        deleteImage();
        passingPOATextField.setText(String.format("%.2f", GirRaftPreset.PASSING_OPEN_AREA_PERCENT));
        resultTextField.setText(String.format("%.2f", GirRaftPreset.PADDL_OPEN_AREA_PERCENT));
        passLabel.setVisible(GirRaftPreset.PADDL_OPEN_AREA_PERCENT >= GirRaftPreset.PASSING_OPEN_AREA_PERCENT);
        failLabel.setVisible(!passLabel.isVisible());
        mainController.showSnackbar("Loaded GirRaft (2025) percent-open-area result");
    }
    ...
    private void handleDragOver(DragEvent event) {
        ...
        if (dragboard.hasFiles() && isValidImageFile(dragboard.getFiles().get(0))) {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        cloud.setOnDragOver(this::handleDragOver);
        cloud.setOnDragDropped(this::handleDragDropped);
    }
}
