...
public class HullBuilderController implements Initializable, ModuleController {
    ...
    private void loadCanoePreset(CanoePreset preset) {
        ...
        setHull(presetHull);
        // Raft Punk's CAD fit contains many short Bézier segments. Hide those
        // construction handles until the user explicitly enters edit mode.
        if (preset == CanoePreset.RAFT_PUNK_2026) {
            hullGraphic.setBezierHandlesVisible(false);
        }
    ...
    private void selectSegment(boolean isNext) {
        // Section navigation is an explicit editing action, so reveal the
        // construction handles if a preset previously hid them.
