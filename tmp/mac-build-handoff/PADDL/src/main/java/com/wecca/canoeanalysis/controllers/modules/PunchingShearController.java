...
public class PunchingShearController implements Initializable, ModuleController {
    ...
    /**
     * Loads the exact values visible in GirRaft's published PADDL Punching
     * Shear output. The source proposal does not include the imported YAML, so
     * the chart is intentionally cleared instead of fabricating a load case.
     */
    private void loadGirRaftPreset() {
        reset();
        hullThicknessTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.structuralThicknessMm()));
        hullWidthTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.nominalMaximumWidthMm()));
        compressiveStrengthTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.COMPRESSIVE_STRENGTH_MPA));
        maxShearTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.PUNCHING_MAXIMUM_SHEAR_N));
        oneWayVfTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.PUNCHING_MAXIMUM_SHEAR_N));
        oneWayVcTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.PUNCHING_ONE_WAY_CAPACITY_N));

        twoWayPCritTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.PUNCHING_CRITICAL_PERIMETER_MM));
        twoWayACritTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.PUNCHING_CRITICAL_AREA_MM2));
        twoWayVfTextField.setText(String.format(Locale.US, "%.2f", GirRaftPreset.PUNCHING_DEMAND_MPA));
        twoWayVc1TextField.setText(String.format(Locale.US, "%.4f", GirRaftPreset.PUNCHING_CAPACITY_1_MPA));
        twoWayVc2TextField.setText(String.format(Locale.US, "%.4f", GirRaftPreset.PUNCHING_CAPACITY_2_MPA));
        twoWayVc3TextField.setText(String.format(Locale.US, "%.4f", GirRaftPreset.PUNCHING_CAPACITY_3_MPA));
        twoWayVcMinTextField.setText(String.format(Locale.US, "%.4f", GirRaftPreset.PUNCHING_GOVERNING_CAPACITY_MPA));

        // Calculated fields remain disabled just as in the original UI; full
        // opacity makes the loaded report values readable.
        ...
        safetyTest1();
        safetyTest2();
        mainController.showSnackbar("Loaded GirRaft (2025) punching-shear values");
    }
