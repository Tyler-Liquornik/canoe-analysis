...
public class PunchingShearController implements Initializable {
    ...
    private void safetyTest(TextField safetyBaselineTextField, TextField checkForSafetyTextField, Label safeLabel, Label unsafeLabel) {
        if (!InputParsingUtils.validateFields(Arrays.asList(safetyBaselineTextField, checkForSafetyTextField))) return;
    ...
    /**
     * Populates the one-way shear force field with the value entered in the max shear field.
    ...
    public void calculateOneWay() {
        if (!InputParsingUtils.validateFields(Arrays.asList(hullThicknessTextField, hullWidthTextField, compressiveStrengthTextField))) return;
    ...
    public void calculateTwoWay() {
        if (!InputParsingUtils.validateFields(Arrays.asList(hullThicknessTextField, compressiveStrengthTextField))) return;
