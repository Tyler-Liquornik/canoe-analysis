...
import com.wecca.canoeanalysis.components.diagrams.DiagramSection;
...
//    }
     ...
     * @return the generated map.
     */
    private static Map<String, Point2D> getDiagramPointMap(List<DiagramSection> intervals, double canoeLength)
    {
        Map<String, Point2D> diagramPoints = new LinkedHashMap<>();
        double rollingMag = 0;
        for (DiagramSection interval : intervals) {
            Point2D start = new Point2D(interval.getSection().getStart(), rollingMag + interval.getMagnitude());
            ...
            Point2D end = new Point2D(interval.getSection().getEnd(), rollingMag + interval.getSlope() * interval.getSection().getLength());
            if (!diagramPoints.containsKey(end.toString())) {
                ...
                rollingMag += interval.getSlope() * interval.getSection().getLength();
            }
        }
    ...
    public static List<Point2D> generateSfdPoints(Canoe canoe)
    {
        ...
        double slope = 0;
        double magnitude = 0;
        List<DiagramSection> intervals = new ArrayList<>();
        ...
        for (int i = 0; i < canoe.getLength() * 100; i ++) {
            ...
            if (distributedLoadStartMap.containsKey(x)) {
                ...
                intervals.add(new DiagramSection(prevX, x, magnitude, slope));
        ...
        intervals.add(new DiagramSection(prevX, canoe.getLength(), magnitude, slope));

        // Sort the list, process them to a map of unique points, and return the points as a list
        intervals.sort(Comparator.comparingDouble(a -> a.getSection().getStart()));
