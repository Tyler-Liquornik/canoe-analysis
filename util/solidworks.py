...
def load_part(solidworks_app: Any, file_path: str) -> Any:
    """
    ...
    model = solidworks_app.OpenDoc6(file_path, doc_type, open_options, "", err, warn)
...
def get_all_vertices(solidworks_model: Any) -> List[Tuple[float, float, float]]:
    """
    Retrieves all vertex coordinates (x, y, z) from all bodies in the part.
    """
    vertices: List[Tuple[float, float, float]] = []
    bodies = solidworks_model.GetBodies2(0, False)
    if bodies is None:
        return vertices
    for body in bodies:
...
def plot_projections(projections: Dict[str, np.ndarray]) -> None:
    ...
    # Plot graphs for each view
    num_plots = len(projections)
    fig, axis = plt.subplots(1, num_plots, figsize=(6 * num_plots, 6))
    if num_plots == 1:
        axis = [axis]

    for ax, (view, pts) in zip(axis, projections.items()):
        # Rotate 90° CCW and shift as a correction to Hull model coordinates
        rot_pts = np.dot(pts, np.array([[0, 1],
                                        [-1, 0]]))
...
def main() -> None:
    ...
    file_path: Optional[str] = find_part_file(current_dir)
    if not file_path:
    ...
    solidworks_app = win32com.client.Dispatch("SldWorks.Application")
    solidworks_app.Visible = True
    solidworks_model = load_part(solidworks_app, file_path)
    vertices: List[Tuple[float, float, float]] = get_all_vertices(solidworks_model)
    if not vertices:
        print("No vertices found in part.")
        return

    # Process projections for XY, XZ, and YZ views.
